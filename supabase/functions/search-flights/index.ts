...
async function searchFlights(request: FlightSearchRequest, token: string) {
  const params = new URLSearchParams({
  ...
  if (!response.ok) {
    console.error(`Flight search error: ${response.statusText}`);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { originLocationCode, destinationLocationCode, departureDate, adults, returnDate } =
      await req.json() as FlightSearchRequest;
